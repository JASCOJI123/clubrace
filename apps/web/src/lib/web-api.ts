...
export const carApi = {
  ...
  mileageLogs(carId: string) {
    return get<{ items: MileageLog[] }>(`/cars/${carId}/mileage`)
  },
  addMileageLog(carId: string, body: { mileageKms: number; date?: string; note?: string }) {
    return post<{ item: MileageLog }>(`/cars/${carId}/mileage`, body)
  },
  deleteMileageLog(id: string) {
    return del<{ ok: boolean }>(`/mileage/${id}`)
  },
}
